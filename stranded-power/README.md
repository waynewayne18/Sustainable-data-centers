...
```
python3 -m pip install geopandas rasterio folium matplotlib pillow
python3 skeleton.py
```
...
`python3 sites_demo.py` → `out/sites.html`

Instead of a heat map, this picks specific candidate sites and lets you
rule them out with checkboxes. Verified: 40 sites, ticking "on the best
farmland" drops it to 13.

That falling number is the demo. A heat map shows a pattern; a shortlist
that shrinks when you add a constraint is an argument.

**Site spacing.** `pick_sites(min_km=25)` keeps sites at least 25km apart.
Without it the top 40 cells are all touching, because good places sit next
to other good places.

**Flags are the checkboxes.** Pass `flags={label: boolean_array}`. Each
becomes a tickbox that removes sites where the array is True. Replace the
percentile thresholds in `sites_demo.py` with real tests once real layers
are in — e.g. ALC grade 1, 2 or 3a for "on the best farmland".

**It survives having no network.** The panel is built before Leaflet is
touched and the map is wrapped in a try/catch. If the venue wifi fails,
the counts, checkboxes and shortlist still work — only the map itself
goes. This was found by testing, not assumed.

Before the day, run `vendor_leaflet()` once and pass the local paths, so
the page doesn't need a CDN at all.
